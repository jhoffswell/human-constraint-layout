...
We compare our recreated visualizations to the origianl layouts and 
...
final figures to better match the original figure.
...
properties such as ethnographic group may reveal additional details about
...
that there were three primary groups involved in the sexual network -- 
...
despite the challenges in creating an informative
...
include a constraint to order each layer by the \texttt{``id''} in order
...
In this layout~(Figure \ref{fig:serengeti-spec}), we define six constraint 
...
we constraint the position of the nodes within the visualization region,
introduce alignments on the nodes, and manage the order with which they
are displayed. For the plant sets, we further introduce clustering on the
nodes to enforce a tighter grouping of the nodes. For this specification,
we define a total of $18$ \projectname constraints, which generates $939$

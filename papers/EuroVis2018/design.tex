...
\feedback{Matt}{It is confusing to me that a constraint and a constraint 
definition don't have a 1-1 mapping. A constraint definition applies multiple 
constraints over a set or multiple sets?}
...
\constraint{Ex: \texttt{partition} by node \emph{depth}}

\feedback{Matt}{I like what you're trying to do with the little code snippets 
in the section headers, but as someone who doesn't know what they mean 
I find them more confusing than helpful}

\feedback{Younghoon}{Next to 4.n. subsection titles: what about showing 
a sample spec in SetCoLa instead of WebCoLa? It might help to connect 
the text to the example codes.}

\todo{It is definitely not clear what these little annotations are so need
to better explain/write those if it is something that we want to try and
keep}
...
\constraint{Ex: node \emph{depth}\texttt{ == 0}}

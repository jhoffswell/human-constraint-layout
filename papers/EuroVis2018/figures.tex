...
\newcommand{\smallTreeExample}{
  \begin{figure}[t!]
    ...
    \includegraphics[width=\columnwidth]{figures/small-tree-example.pdf}
      ...
      The \projectname specification and data for a tree with six nodes. 
      Nodes are split into three sets based on their \texttt{depth} and 
      aligned. A new set definition uses composition to include only the 
      ``layer'' set (the elements of which are the three sets previously 
      partitioned by depth) and orders each layer by its depth to form the tree.}}
    \vspace{-20px}
  \end{figure}
}
...
\newcommand{\smallTreeExampleWebCoLa}{
  \begin{figure}[t!]
    ...
    \includegraphics[width=\columnwidth]{figures/small-tree-webcola.pdf}
      ...
      nodes. The nodes are split into three sets based on their \texttt{depth} and 
      aligned. A new set definition uses composition to include only the 
...
\newcommand{\serengetiLayoutColumn}{
  \begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/serengeti-layout-column.pdf}
    \vspace{-15px} {\caption{\label{fig:serengeti-layout} The layout for
        the Serengeti food web from (a) Baskerville et al.~\cite{baskerville2011spatial}
        as compared to (b) the layout recreated with \projectname.
        Nodes are grouped by trophic level clusters produced from a Bayesian
...
\newcommand{\serengetiHybrid}{
  \begin{figure}[t!]
    \centering
    \includegraphics[width=\columnwidth]{figures/serengeti-hybrid.pdf}
    \vspace{-15px} {\caption{\label{fig:serengeti-spec}\label{fig:serengeti-layout} 
        The customized layout for the Serengeti food web from (a)~Baskerville~et~al.~\cite{baskerville2011spatial}
        as compared to (b)~the~layout recreated with \projectname.
        Nodes are layered by trophic level (e.g.,~plant, herbivore, carnivore) and clustered into groups using a 
        Bayesian analysis method. (c)~The \projectname specification.}}
    \vspace{-30px}
  \end{figure}
}
...
\newcommand{\syphilisLayoutColumn}{
  \begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/syphilis-layout-column.pdf}
    \vspace{-15px} {\caption{\label{fig:syphilis-layout} The layout for the
        syphilis social network from (a) Rothenberg et
        al.~\cite{rothenberg1998using}. (b) We recreated and improved the
        layout in \projectname by introducing additional padding, alignment,
        ...
        not associated with any of these ``core'' groups are positioned above 
        the others. In both figures, individuals diagnosed with syphilis 
        during the outbreak are labeled with an ``S.''}}
    \vspace{-20px}
  \end{figure}
}
...
\newcommand{\syphilisSpec}{
  \begin{figure}[t]
    ...
    \vspace{-15px} {\caption{\label{fig:syphilis-spec} The
        ...
        number of sets produced (\figuregreen{green}), the number of WebCoLa constraints
        generated for the final layout (\figureblue{blue}), and the behavior of \projectname
        constraints not directly converted to WebCoLa constraints (\figurepurple{purple}).
    }}
    \vspace{-20px}
  \end{figure}
}
...
\newcommand{\constraintsFigure}{
  \begin{figure*}[t]
    ...
    \vspace{-15px} {\caption{\label{fig:constraints} The number of nodes,
        ...
        list the number of definitions or constraints
        written by the user. We compare the number of \textbf{\projectname Constraints}
        to the number of \textbf{WebCoLa Constraints} generated by the 
        \projectname compiler to determine the factor by which the number of constraints increases (\textbf{Ratio}).}} 
    \vspace{-20px}
  \end{figure*}
}
